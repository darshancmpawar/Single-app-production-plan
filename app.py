...
tab1, tab2 = st.tabs(["Config Client", "Generate Production Plan"])

with tab1:
    st.subheader("Client Logic Configuration")
    cfg = st.session_state[cfg_key]

    c1, c2 = st.columns(2)
    with c1:
        cfg["client_name"] = st.text_input("Client Name", value=cfg.get("client_name", sel), key=k("cfg_client_name"))
    with c2:
    ...
    slab_df = pd.DataFrame(slab_default, columns=["min_mg", "max_mg", "adjustment_pct"])
    ...
    if st.button("Save Client Config", key=k("cfg_save_btn")):
        if not cfg.get("client_name", "").strip():
            ...
            st.success("Client configuration saved.")
